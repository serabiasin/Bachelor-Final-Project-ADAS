...
def depthwise_conv(inputs, pointwise_filter, alpha, depth_mutiplier=1, strides=(1, 1), name=None):
    ...
    x = BatchNormalization(
        momentum=0.99, epsilon=0.00001, name=name+"/DW_BN")(x)
    x = ReLU(6., name=name+"/DW_RELU")(x)

    x = Conv2D(pointwise_filter, (1, 1), strides=(1, 1),
               padding='same', use_bias=False, name=name+"/PW")(x)
    x = BatchNormalization(
        momentum=0.99, epsilon=0.00001, name=name+"/PW_BN")(x)
...
def SSD300(img_size, n_classes, l2_reg=5e-4,
    ...
    temp = tf.keras.applications.MobileNet(
        input_shape=(img_size), include_top=False, weights=None)
    base_network = Model(
        inputs=temp.input, outputs=temp.get_layer("conv_pw_13_relu").output)

    conv6_1 = ssd_conv(base_network.get_layer(
        "conv_pw_13_relu").output, 256, (1, 1), padding='same', name='conv6_1')

    conv6_2 = ssd_conv(conv6_1,  512, (3, 3), strides=(
        2, 2), padding='same', name='conv6_2')

    conv7_1 = ssd_conv(conv6_2, 128, (1, 1), padding='same', name='conv7_1')
    conv7_2 = ssd_conv(conv7_1, 256, (3, 3), strides=(
        2, 2), padding='same', name='conv7_2')

    conv8_1 = ssd_conv(conv7_2, 128, (1, 1), padding='same', name='conv8_1')
    conv8_2 = ssd_conv(conv8_1, 256, (3, 3), strides=(
        2, 2), padding='same', name='conv8_2')

    conv9_1 = ssd_conv(conv8_2, 128, (1, 1), padding='same', name='conv9_1')
    conv9_2 = ssd_conv(conv9_1, 256, (3, 3), strides=(
        2, 2), padding='same', name='conv9_2')

    conv4_3_norm = Normalize(
        classes-1, name='conv4_3_norm')(base_network.get_layer('conv_pw_11_relu').output)

    # Build "n_classes" confidence values for each box. Ouput shape: (b, h, w, n_boxes*n_classes)
    conv4_3_norm_mbox_conf = Conv2D(
        n_boxes[0] * classes, (3, 3), padding='same',  name='conv4_3_norm_mbox_conf')(conv4_3_norm)
    fc7_mbox_conf = Conv2D(n_boxes[1] * classes, (3, 3), padding='same',
                           name='fc7_mbox_conf')(base_network.get_layer("conv_pw_13_relu").output)
    conv6_2_mbox_conf = Conv2D(
        n_boxes[2] * classes, (3, 3), padding='same', name='conv6_2_mbox_conf')(conv6_2)
    conv7_2_mbox_conf = Conv2D(
        n_boxes[3] * classes, (3, 3), padding='same',  name='conv7_2_mbox_conf')(conv7_2)
    conv8_2_mbox_conf = Conv2D(
        n_boxes[4] * classes, (3, 3), padding='same', name='conv8_2_mbox_conf')(conv8_2)
    conv9_2_mbox_conf = Conv2D(
        n_boxes[5] * classes, (3, 3), padding='same', name='conv9_2_mbox_conf')(conv9_2)

    # Build 4 box coordinates for each box. Output shape: (b, h, w, n_boxes * 4)
    conv4_3_norm_mbox_loc = Conv2D(
        n_boxes[0] * 4, (3, 3), padding='same', name='conv4_3_norm_mbox_loc')(conv4_3_norm)
    fc7_mbox_loc = Conv2D(n_boxes[1] * 4, (3, 3), padding='same',
                          name='fc7_mbox_loc')(base_network.get_layer("conv_pw_13_relu").output)
    conv6_2_mbox_loc = Conv2D(
        n_boxes[2] * 4, (3, 3), padding='same', name='conv6_2_mbox_loc')(conv6_2)
    conv7_2_mbox_loc = Conv2D(
        n_boxes[3] * 4, (3, 3), padding='same',  name='conv7_2_mbox_loc')(conv7_2)
    conv8_2_mbox_loc = Conv2D(
        n_boxes[4] * 4, (3, 3), padding='same', name='conv8_2_mbox_loc')(conv8_2)
    conv9_2_mbox_loc = Conv2D(
        n_boxes[5] * 4, (3, 3), padding='same', name='conv9_2_mbox_loc')(conv9_2)
    ...
    fc7_mbox_priorbox = AnchorBoxes(img_size=img_size, min_size=anchors[1], max_size=anchors[2], aspect_ratios=[2, 3],
                                    variances=variances, name='fc7_mbox_priorbox')(base_network.get_layer("conv_pw_13_relu").output)
