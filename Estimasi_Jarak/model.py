...
def create_model(existing='', is_twohundred=False, is_halffeatures=True):
    ...
    if len(existing) == 0:
        ...
        def upproject(tensor, filters, name, concat_with):
            up_i = BilinearUpSampling2D(
                (2, 2), name=name+'_upsampling2d')(tensor)
            # Skip connection
            up_i = Concatenate(
                name=name+'_concat')([up_i, base_model.get_layer(concat_with).output])
            up_i = Conv2D(filters=filters, kernel_size=3, strides=1,
                          padding='same', name=name+'_convA')(up_i)
            up_i = LeakyReLU(alpha=0.2)(up_i)
            up_i = Conv2D(filters=filters, kernel_size=3, strides=1,
                          padding='same', name=name+'_convB')(up_i)
            up_i = LeakyReLU(alpha=0.2)(up_i)
            return up_i
        ...
        decoder = upproject(decoder, int(decode_filters/2),
                            'up1', concat_with='conv_dw_13')
        decoder = upproject(decoder, int(decode_filters/4),
                            'up2', concat_with='conv_dw_10')
        decoder = upproject(decoder, int(decode_filters/8),
                            'up3', concat_with='conv_dw_7')
        decoder = upproject(decoder, int(decode_filters/16),
                            'up4', concat_with='conv1_relu')
        if False:
            decoder = upproject(decoder, int(
                decode_filters/32), 'up5', concat_with='input_1')

        # Extract depths (final layer)
        conv3 = Conv2D(filters=1, kernel_size=3, strides=1,
                       padding='same', name='conv3')(decoder)
