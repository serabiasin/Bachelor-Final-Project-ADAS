...
def SSD300(img_size, n_classes, l2_reg=5e-4,
    ...
    temp = tf.keras.applications.VGG16(
        input_shape=(img_size), include_top=False, weights=None)
    base_network = Model(
        inputs=temp.input, outputs=temp.get_layer("block5_pool").output)

    conv6_1 = ssd_convolution(base_network.get_layer(
        "block4_conv1").output, 256, (1, 1), padding='same', name='conv6_1')
    ...
    conv4_3_norm = Normalize(
        classes-1, name='conv4_3_norm')(base_network.get_layer('block3_pool').output)
    ...
    fc7_mbox_conf = Conv2D(n_boxes[1] * classes, (3, 3), padding='same', name='fc7_mbox_conf')(base_network.get_layer("block4_pool").output)
    ...
    fc7_mbox_loc = Conv2D(n_boxes[1] * 4, (3, 3), padding='same',
                          name='fc7_mbox_loc')(base_network.get_layer("block4_pool").output)
    ...
    fc7_mbox_priorbox = AnchorBoxes(img_size=img_size, min_size=anchors[1], max_size=anchors[2],aspect_ratios=[2,3],
                                    variances=variances, name='fc7_mbox_priorbox')(base_network.get_layer("block4_pool").output)
