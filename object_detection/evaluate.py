...
parser.add_argument('--input_result', default='experiment_result.txt',
                    type=str, help='File directory resulted by predict_text.py')
...
def compute_average_precision(init_file, result_file):
    ...
    for index in range(len(result_lines)):
        ...
        original_bboxes = np.array(
            [np.array(list(map(int, box.split(",")))) for box in original[1:]])
        result_bboxes = np.array(
            [np.array(list(map(int, box.split(",")))) for box in result[1:]])
...
if __name__ == "__main__":
    compute_average_precision(
        init_file="val.txt", result_file=args.input_result)
    union_commtity(init_file="val.txt", result_file=args.input_result)
